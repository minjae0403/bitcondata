...
def create_connection():
    return mysql.connector.connect(
        ...
        database='crypto_min_db'  # Replace with your database name
    )
...
def get_data(start_date, end_date, symbol):
    ...
    while start < end:
        ...
        if not js:
            break
        data.extend(js)
        start = js[-1][0] + 60000  # 1분 단위로 다음 데이터를 가져오기
...
# Start and end dates
years = range(2021, 2023)  # 바이낸스 API에서 1분 단위 데이터를 너무 오래된 기간에 대해서는 제공하지 않으므로 최근 1~2년으로 제한
